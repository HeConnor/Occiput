...
from ...Core.NiftyPy_wrap import SPECT_project_parallelholes, SPECT_backproject_parallelholes, has_NiftyPy
from ...Core.Core import Image3D
...
class SPECT_Static_Scan(object):
    ...
    def get_gantry_angular_positions(self): 
        return (self._p_gantry_angular_position_first,
                self._p_gantry_angular_position_last, self._p_gantry_angular_positions)
    ...
    def project(self, activity, attenuation=None, cameras=None, psf=None, subsets_array=None): 
        ...
        if psf is None:
            psf = self._psf
        proj = SPECT_project_parallelholes(activity, cameras, attenuation, psf,
                                           self._background_activity, self._background_attenuation,
                                           self._use_gpu, self._truncate_negative)
    ...
    def backproject(self, projection, attenuation=None, cameras=None, psf=None, subsets_array=None):
        ...
        if psf is None:
            psf = self._psf
        backproj = SPECT_backproject_parallelholes(projection, cameras, attenuation, psf,
                                                   self._background_activity, self._background_attenuation,
                                                   self._use_gpu, self._truncate_negative)
    ...
    def __make_probabilistic_graphical_model(self): 
        ...
        self.graph = Graph({'nodes': [{'name': 'activity', 'type': 0},
                                      {'name': 'counts', 'type': 0}],
    ...
    def _compute_normalisation(self): 
        ...
        self._norm = self.backproject(ones((self._p_n_pix_x, self._p_n_pix_y, self._p_gantry_angular_positions),
                                           dtype=float32, order="F")).data
    ...
    def estimate_activity(self, attenuation=None, psf=None,
                          iterations=DEFAULT_ITERATIONS, subset_size=DEFAULT_SUBSET_SIZE,
                          subset_mode='random', method="EM",
                          m=32, factr=0.01, pgtol=1e-16, maxfun=10000, smoothing=0.0, activity=None):
        ...
        if method == "EM":
            ...
            for i in range(iterations): 
                ...
                if subsets_array is not None: 
                    ...
                    norm = self.backproject(ones((self._p_n_pix_x, self._p_n_pix_y, subset_size),
                                                 dtype=float32, order="F"), attenuation=attenuation,
                                            psf=psf, subsets_array=subsets_array).data
                ...
                # print "Iteration: %d    max act: %f    min act: %f
                # max proj: %f    min proj: %f
                # max norm: %f    min norm: %f"%(i, activity.max(), activity.min(),
                # proj.max(), proj.min(), norm.data.max(), norm.data.min() )
            progress_bar.set_percentage(100.0)
        elif method == "LBFGS":
    ...
    def get_likelihood(self, activity, activity_size, smoothing=0.0):
        """
        Returns the likelihood value - given the activity.
        This at the moment implements the Poisson likelihood only.
        """
        eps = 1e-16
    ...
    def get_gradient_activity(self, activity, activity_size, smoothing=0.0):
        """Returns the derivative of the log-likelihood with respect to the activity.
        At the moment, this implements only the Poisson likelihood.
        """
        eps = 1e-16
        sinogram = self._measurement
        psf = self._psf
    ...
    def get_likelihood_log(self, activity_log, activity_size, smoothing=0.0): 
        ...
        This at the moment implements the Poisson likelihood only.
        This is useful to optimize the likelihood or the posterior
        with respect to the log of the activity - e.g. to include the non negativity constraint.
        """
    ...
    def get_gradient_log_activity(self, activity_log, activity_size, smoothing=0.0):
        """Returns the derivative of the log-likelihood with respect to the log of the activity.
        At the moment, this implements only the Poisson likelihood.
        This is useful to optimize the likelihood or the posterior with respect to thelog of the activity
        - e.g. to include the non negativity constraint.
        """
    ...
    def volume_render(self, volume, scale=1.0):
        # FIXME: use the VolumeRenderer object in occiput.
        # Visualization (improve it), the following is a quick fix:
    ...
    def set_measurement(self, measurement): 
        if not (self._p_n_pix_x == measurement.shape[0] and self._p_n_pix_y == measurement.shape[1]
                 and self._p_gantry_angular_positions == measurement.shape[2]):
            raise UnexpectedParameter('Measurement size is not compatible with n_pix_x, n_pix_y, '
                                      'gantry_angular_positions. ')
    ...
    def _make_svg(self): 
        ...
        # line = dwg.add(dwg.polyline([(10, 10), (10, 100), (100, 100),
        # (100, 10), (10, 10)],stroke='black', fill='none'))
