...
from ...Core.NiftyPy_wrap import PET_compress_projection, PET_uncompress_projection, PET_initialize_compression_structure #, PET_compress_projection_array
...
__all__ = ["DEFAULT_BINNING",
           ...
           "display_PET_Projection_geometry",]      # "PET_compress_projection_array"]
...
class Binning():
    ...
    def __init__(self, parameters=None):
        ...
        if parameters is None:
            ...
            raise UnknownParameter('Parameter %s specified for the construction of Binning is not compatible. '
                                   % str(parameters))
    ...
    def load_from_dictionary(self, dictionary):
        ...
        self.size_u = dictionary['size_u']                     # Size of the detector plane, axis u, [a dimensional]
        self.size_v = dictionary['size_v']                     # Size of the detector plane, axis v,  [a dimensional]
...
class PET_Projection_Sparsity():
    """
    Represents the sparsity pattern of projection data and implements methods
    to compress and uncompress data according to the sparsity pattern of self.
    ...
    def is_uncompressed(self):
        """
        Returns True if the sparsity pattern represents
        an uncompressed sampled projection.
        """
        ...
        for iax in range(N_axial):
            for iaz in range(N_azimuthal):
                if subset_matrix[iaz, iax]:
                    ...
                    locations_return[:, offsets_return[0, ii - 1]:
                                        offsets_return[0, ii - 1] + size] = \
...
class PET_Projection():
    """
    Sparse PET projection object.
    If offsets=None or locations=None in the constructor, the projection is uncompressed.
    """
    def __init__(self, binning, data=None, offsets=None, locations=None, time_bins=None, subsets_matrix=None):
        ...
        sparsity = PET_Projection_Sparsity(binning.N_axial, binning.N_azimuthal,
                                           binning.N_u, binning.N_v, offsets, locations)
        if subsets_matrix is not None:
    ...
    def compress_as_self(self, projection):
        ...
        according to the sparsity of self.
        ...
        return PET_Projection(self.get_binning(), compressed_data,
                              self.sparsity.offsets, self.sparsity.locations, self.get_time_bins())
    ...
    def compress_self(self):
        if self.is_compressed():
            ...
            print("Not implemented. Please implement me. "
                  "This needs low level implementation. Right now sparsity comes only from listmode data. ")
    ...
    def uncompress_self(self):
        ...
        if self.is_uncompressed():
            return self
        uncompressed_data = self.sparsity.uncompress_data(self.data)
        return PET_Projection(self.get_binning(), uncompressed_data,
                              offsets=None, locations=None, time_bins=self.get_time_bins())
    ...
    def get_projection_info(self):
        """
        Returns information related to the compression,
        such as the compression ratio. It returns a dictionary.
        """
    ...
    def to_nd_array(self):
        ...
        if not self.is_uncompressed():
            ...
            uncompressed = self
        shape = (uncompressed.sparsity.N_axial, uncompressed.sparsity.N_azimuthal,
                 uncompressed.sparsity.N_u, uncompressed.sparsity.N_v)
    ...
    def display(self, axial=True, azimuthal=None, index=0, scale=None, open_browser=False):
        ...
        if axial is None and azimuthal is None:
    ...
    def crop(self, bins_range):
        ...
        """
        if self.is_compressed():
            print("Cropping of PET_Projection currently only works with uncompressed data. "
                  "Please implement for compressed data. ")
            return
        A = bins_range[0]
        B = bins_range[1]
        data = self.data[:, :, A: B, :]
    ...
    def get_subset(self, subsets_matrix):
        ...
        return PET_Projection(self.binning, data, self.sparsity.offsets,
                              self.sparsity.locations, self.time_bins, subsets_matrix)
