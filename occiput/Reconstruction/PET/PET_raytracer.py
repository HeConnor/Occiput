...
__all__ = ["DEFAULT_PROJECTION_PARAMETERS",
...
class ProjectionParameters:
    ...
    def __init__(self, parameters=None):
        ...
        if parameters is None: 
            ...
            if len(parameters) == len(self.default_parameters.keys()): 
                ...
                raise UnknownParameter('Parameter %s specified for ProjectionParameters is not compatible. '
                                       % str(parameters))
        else: 
            raise UnknownParameter('Parameter %s specified for ProjectionParameters is not compatible. '
                                   % str(parameters))
    ...
    def load_from_dictionary(self, dictionary):
        ...
        # distance between consecutive points along a line
        # when computing line integrals (this is in the same unit measure
        # as the size of the imaging volume (activity_size and attenuation_size))
        ...
        # (This is meant to remove eventual unwanted small negative values
        # due to FFT-based smoothing within the projection algorithm
        # - note that numerical errors may produce small negative numbers
        # when doing FFT-IFFT even if the function is all positive )
        ...
        # Direction parameter for the projector and b-proj.,
        # it affects performance only; 2 and 5 are normally the best values.
        self.direction = dictionary['direction']

        # Number of blocks per thread for the GPU accelerated projector and b-proj.
        # It affects performance only. Typical values: 256, 512, 768
